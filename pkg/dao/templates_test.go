...
func (s *TemplateSuite) TestListFilterExtendedRelease() {
	...
	assert.Equal(s.T(), int64(1), total)
	assert.Len(s.T(), responses.Data, 1)
	assert.Equal(s.T(), found[2].ExtendedRelease, responses.Data[0].ExtendedRelease)

	// Test filter by extended_release_version
	filterData = api.TemplateFilterData{ExtendedReleaseVersion: found[1].ExtendedReleaseVersion}
	responses, total, err = templateDao.List(context.Background(), orgIDTest, false, api.PaginationData{Limit: -1}, filterData)
	assert.NoError(s.T(), err)
	assert.Equal(s.T(), int64(1), total)
	assert.Len(s.T(), responses.Data, 1)
	assert.Equal(s.T(), found[1].ExtendedReleaseVersion, responses.Data[0].ExtendedReleaseVersion)
}
